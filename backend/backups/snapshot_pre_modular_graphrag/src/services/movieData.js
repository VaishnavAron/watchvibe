...
export function formatNeo4jMovie(m) {
  return {
    ...
    year: m.year || null,
    posterUrl: m.poster_url || m.posterUrl || 'https://placehold.co/320x480/1f2937/f9fafb?text=No+Poster',
    backdropUrl: m.backdrop_url || m.backdropUrl || null,
    overview: m.overview || '',
...
export async function getMoviesByTitles(titles) {
  ...
  try {
    ...
    for (const r of result.records) {
      ...
      if (raw && raw.title) {
        map.set(raw.title.toLowerCase().trim(), {
          title: raw.title,
