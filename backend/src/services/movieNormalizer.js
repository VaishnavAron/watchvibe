...
export function normalizeMovieRecord(record, index = 0) {
  ...
  const rawPoster = clean(movie.posterUrl) || clean(movie.poster_url);
  const posterUrl = rawPoster && !rawPoster.includes('placehold.co')
    ? rawPoster.replace(/^(https?:\/\/image\.tmdb\.org\/t\/p\/[^\/]+)+(https?:\/\/image\.tmdb\.org\/t\/p\/[^\/]+)/, '$2')
